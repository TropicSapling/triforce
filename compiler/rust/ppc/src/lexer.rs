			...
			if escaping {
				...
				if item == "/" {
					...
					string.kind = match item {
						...
						"as" | "async" | "break" | "continue" | "export" | "foreach" | "from" | "goto" | "import" | "in" | "match" | "receive" | "repeat" | "select" | "send" | "to" | "type" | "until" | "when" | "while" => Kind::Reserved(item.to_string(), RefCell::new(Vec::new())),
